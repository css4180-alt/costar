...
- **공동 출연(Match)**: 사진 한 장을 올리면 **여러 얼굴을 한 번에 검출·식별**(`DetectFaces`
  → 얼굴별 crop → `SearchFacesByImage`)하고, 식별된 인물들이 함께 출연한 작품을
  **집합 교집합**으로 찾는다. 얼굴 박스·이름·유사도를 이미지 위에 표시한다.
  (인물을 직접 선택해 교집합을 보는 보조 모드도 제공.)
